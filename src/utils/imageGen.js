...
async function generateOrderImage(order) {
  ...
  if (thumb) {
    // Circular clipped thumbnail top-right
  ...
  // ── Order type icon + title
  const typeEmoji = order.type === 'Gold' ? '💰' : order.type === 'Gems' ? '💎' : '🧱';
  ...
  // ── Order code pill + status badge
  // Code pill
  ctx.fillStyle = '#1A1A2E';
  roundRect(ctx, 28, 84, 160, 28, 6);
  ctx.fill();
  ...
  roundRect(ctx, 200, 84, 110, 28, 14);
  ctx.fill();
  ctx.strokeStyle = statusConf.color;
  ctx.lineWidth = 1;
  roundRect(ctx, 200, 84, 110, 28, 14);
  ctx.stroke();
  // Status dot
  ...
  divGrad.addColorStop(0, accent + '88');
  ...
  for (const [icon, label, value] of rows) {
    ...
    roundRect(ctx, 28, rowY - 18, 380, 30, 6);
    ctx.fill();

    ctx.font = '14px sans-serif';
    ctx.fillStyle = '#AAAAAA';
    ctx.fillText(`${icon}  ${label}`, 40, rowY);

    // Value pill
    ctx.fillStyle = accent + '22';
    const valW = ctx.measureText(value).width + 20;
    roundRect(ctx, 420 - valW, rowY - 16, valW, 24, 5);
    ctx.fill();
    ctx.font = 'bold 13px monospace';
    ctx.fillStyle = '#FFFFFF';
    ctx.fillText(value, 430 - valW, rowY);

    rowY += 36;
  }

  // ── Stock section (right side)
  ...
  const bigNum = order.type === 'Gems'
    ...
    : formatGold(order.remainingQuantity).replace(' Gold', '').replace(' gold', '');
  ...
  ctx.fillText(bigNum, sx + sw / 2, sy + 105);
  ctx.restore();
  ...
  ctx.fillText(unitLabel, sx + sw / 2, sy + 124);

  // Divider
  ctx.fillStyle = accent + '33';
  ctx.fillRect(sx + 14, sy + 135, sw - 28, 1);
  ...
  roundRect(ctx, sx + 14, sy + 146, sw - 28, 36, 18);
  ctx.fill();
  ...
  ctx.shadowColor = stockColor;
  ctx.shadowBlur = 8;
  ctx.beginPath();
  ctx.arc(sx + 30, sy + 164, 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowBlur = 0;
  ...
  ctx.fillStyle = stockColor;
  ctx.textAlign = 'left';
  ctx.fillText(`${stockLabel} (${pct}%)`, sx + 42, sy + 169);
  ctx.textAlign = 'left';
